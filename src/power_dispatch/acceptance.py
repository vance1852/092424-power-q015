...
def _ctx(service: SupplyService, user_id: str):
...
def run(workspace: Path) -> dict[str, object]:
    ...
    plan, dispatch, risk, audit = (_ctx(service, name) for name in ("plan", "dispatch", "risk", "audit"))
    ...
    # 送电是敏感操作：调度员申请 -> 风险岗第二人复核批准 -> 调度员持一次性票据执行。
    review = service.access.request_review(dispatch, ReviewGate(
        scope="transfer.dispatch", entity_type="nomination", entity_id="nom-001",
        expected_version=2, payload={"transfer_id": "transfer-001", "lot_id": "lot-001"},
    ))
    decision = service.access.decide_review(risk, review["ticket_id"], True, "负荷与库存核对无误")
    transfer = service.dispatch_transfer(
        dispatch, "transfer-001", "nom-001", "lot-001", 2, review["ticket_id"]
    )
    ...
    result = {
        ...
        "access_audit": service.access.audit_chain(audit),
